...
public class Sonidos {
    ...
    private void initComponents() {
        for(var i = 0; i < cantidadDeSonidos; i++) {
    ...
    public boolean isPlaying(int sonido) {
        return sonido < sonidos.size() && sonidos.get(sonido).isPlaying();
    }

    public void pausar() {
        for (var sonido : sonidos)
            sonido.pause();
    }
    ...
    public void detener(int... sonido) {
        if (sonidos.isEmpty())
            return;
        for(var indice : sonido) {
            var s = sonidos.get(indice);
            if(s.isPlaying()) {
                s.stop();
            }
        }
