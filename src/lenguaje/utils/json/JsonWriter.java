 ...
 * <code>JsonWriter.toJson(employee)</code>. This will convert the passed in
 * 'employee' instance into a JSON String.</li>
 * <li>Using streams:
 * <pre>     JsonWriter writer = new JsonWriter(stream);
 *     writer.write(employee);
 *     writer.close();</pre> This will write the 'employee' object to the passed in
 * OutputStream.
 * </li>
 * <p>
...
public class JsonWriter implements Closeable, Flushable {
    ...
    private void traceReferences(Object root) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (clazz.isArray()) {
                ...
                if (!JsonReader.isPrimitive(compType)) {   // Speed up: do not traceReferences of primitives, they cannot reference anything
    ...
    private static void traceFields(LinkedList<Object> stack, Object obj) {
        ...
        for (Field field : fields.values()) {
            try {
                if (JsonReader.isPrimitive(field.getType())) {    // speed up: primitives cannot reference another object
                    continue;
                }
    ...
    private boolean writeOptionalReference(Object obj) throws IOException {
        ...
        if (_objVisited.containsKey(obj)) {    // Only write (define) an object once in the JSON stream, otherwise emit a @ref
            String id = getId(obj);
    ...
    private void writeImpl(Object obj, boolean showType) throws IOException {
        ...
        } else if (obj instanceof JsonObject) {   // symmetric support for writing Map of Maps representation back as identical JSON format.
    ...
    private boolean doesValueTypeMatchFieldType(Class type, String fieldName, Object value) {
        if (type != null) {
            ...
            if (field == null) {   // Can happen when the 'same' class is different in two different JVMs.
                return false;
            }
     ...
     * @param showType boolean true means show the "
     * @type" field, false eliminates it. Many times the type can be dropped
     * because it can be inferred from the field or array type.
     * @throws IOException if an error occurs writing to the output stream.
     */
