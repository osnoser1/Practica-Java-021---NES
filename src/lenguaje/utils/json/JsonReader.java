 ...
 * Usages:
 * <ul><li>
 * Call the static method: <code>JsonReader.objectToJava(String json)</code>.
 * This will return a typed Java object graph.</li>
 * <li>
 * Call the static method: <code>JsonReader.jsonToMaps(String json)</code>. This
 * will return an untyped object representation of the JSON String as a Map of
 * Maps, where the fields are the Map keys, and the field values are the
 * associated Map's values. You can call the JsonWriter.objectToJava() method
 * with the returned Map, and it will serialize the Graph into the identical
 * JSON stream from which it was read.
 * <li>
 * Instantiate the JsonReader with an InputStream:
 * <code>JsonReader(InputStream in)</code> and then call
 * <code>readObject()</code>. Cast the return value of readObject() to the Java
 * class that was the root of the graph.
 * </li>
 * <li>
 * Instantiate the JsonReader with an InputStream:
 * <code>JsonReader(InputStream in, true)</code> and then call
...
public class JsonReader implements Closeable {
    ...
    protected Object readIfMatching(Object o, Class compType, LinkedList<JsonObject<String, Object>> stack) throws IOException {
        ...
        if (!isJsonObject && compType == null) {   // If not a JsonObject (like a Long that represents a date, then compType must be set)
            return null;
        }
     ...
     * Java object.
     *
     * @param root JsonObject instance that was the root object from the JSON
     ...
     *
     * @param root JsonObject reference to a Map-of-Maps representation of the
    ...
    private void traverseArray(LinkedList<JsonObject<String, Object>> stack, JsonObject<String, Object> jsonObj) throws IOException {
        ...
        if (byte.class.equals(compType)) {   // Handle byte[] special for performance boost.
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (element == null) {
            ...
            } else if (element == EMPTY_OBJECT) {    // Use either explicitly defined type in ObjectMap associated to JSON, or array component type.
                ...
                if (char[].class.equals(compType)) {   // Specially handle char[] because we are writing these
                ...
                if (ref != null) {    // Connect reference
                    ...
                    if (refObject.target != null) {   // Array element with @ref to existing object
                    ...
                    Array.set(array, i, arrayElement);
                    if (!isPrimitive(arrayElement.getClass())) {    // Skip walking primitives, primitive wrapper classes,, Strings, and Classes
                        stack.addFirst(jsonObject);
                    }
                }
            } else {   // Setting primitive values into an Object[]
            ...
            } else // if (element instanceof JsonObject)
            {
                ...
                if (ref != null) {
                    ...
                    if (refObject.target != null) {
                        ...
                        if (isList) {   // Indexable collection, so set 'null' as element for now - will be patched in later.
                            col.add(null);
                        }
                    }
                } else {
    ...
    private void traverseFieldsNoObj(LinkedList<JsonObject<String, Object>> stack, JsonObject<String, Object> jsonObj) throws IOException {
        ...
        for (Map.Entry<String, Object> e : jsonObj.entrySet()) {
            ...
            if (key.charAt(0) == '@') {   // Skip our own generated fields
                continue;
            }
            ...
            } else if (value.getClass().isArray()) {    // LHS of assignment is an [] field or RHS is an array and LHS is Object (Map)
     ...
     * @param jsonObj a Map-of-Map representation of the current object being
     ...
     * @param jsonObj a Map-of-Map representation of the current object being
     * examined (containing all fields).
     * @param field a Java Field object representing where the jsonObj should be
     * converted and stored.
     * @param rhs the JSON value that will be converted and stored in the
    ...
    private void assignField(LinkedList<JsonObject<String, Object>> stack, JsonObject jsonObj, Field field, Object rhs) throws IOException {
        ...
        try {
            ...
            if (rhs == null) {
                ...
                JsonObject<String, Object> jsonArray = new JsonObject<String, Object>();
                if (char[].class.equals(fieldType)) {   // Specially handle char[] because we are writing these
     ...
     * jsonObj to provide enough hints to get the right class instantiated. It
     * is not populated when returned.
     * @throws IOException for stream errors or parsing errors.
     */
    private Object createJavaObjectInstance(Class clazz, JsonObject jsonObj) throws IOException {
        ...
        if (type != null) {    // @type is explicitly set, use that as it always takes precedence
            ...
            if (c.isArray()) {    // Handle []
                ...
                } else if ("java.util.Arrays$ArrayList".equals(c.getName())) {	// Special case: Arrays$ArrayList does not allow .add() to be called on it.
                    mate = new ArrayList();
                } else {
                    mate = newInstance(c);
                }
            ...
            } else if ("java.util.Arrays$ArrayList".equals(clazz.getName())) {	// Special case: Arrays$ArrayList does not allow .add() to be called on it.
                mate = new ArrayList();
            } else {
     ...
     * a decimal number or integer. This choice allows all smaller types (Float,
     * int, short, byte) to be represented as well.
     * @throws IOException for stream errors or parsing errors.
     */
    ...
    private static Object newPrimitiveWrapper(Class c, Object rhs) throws IOException {
        ...
        if (c.equals(Boolean.class) || c.equals(boolean.class)) {    // Booleans are tokenized into Boolean.TRUE or Boolean.FALSE
     ...
     * Field returned is cached so that it is only obtained via reflection once.
     * @throws IOException for stream errors or parsing errors.
     */
    ...
    private void patchUnresolvedReferences() throws IOException {
        ...
        while (i.hasNext()) {
            ...
            if (ref.index >= 0) {    // Fix []'s and Collections containing a forward reference.
                ...
                } else if (objToFix instanceof Collection) {   // Add element (since it was not indexable, add it to collection)
     ...
     *
     * If _noObjects==true, then move @keys to keys and @values to values and
    ...
    private static char[] toChars(int codePoint) {
        if (codePoint < 0 || codePoint > MAX_CODE_POINT) {    // int UTF-8 char must be in range
            throw new IllegalArgumentException("value ' + codePoint + ' outside UTF-8 range");
        }

        if (codePoint < MIN_SUPPLEMENTARY_CODE_POINT) {    // if the int character fits in two bytes...
     ...
     * PushbackReader. This is due to this class not using syncrhonization as it
     * is not needed.
     */
